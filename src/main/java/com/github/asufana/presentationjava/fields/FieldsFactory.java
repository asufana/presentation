...
public class FieldsFactory {
        ...
        Stream.of(fields)
                ...
                .forEach(field -> {
                    ...
                    if (hasChild(field)) {
                        ...
                        final List<java.lang.reflect.Field> nextParentFields = createParentFields(parentFields, field);
                        discovery(nextParentFields, nextClass, nextInstance, discoveredFields);
                    }
                    //子要素なければクラス保持
                    else {
                        final Field f = new Field(parentFields, field, targetInstance);
     ...
     * インスタンス値の取得
     */
    protected static Object value(final Object instance, final java.lang.reflect.Field field) {
        try {
